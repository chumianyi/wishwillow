...
class Mesh {
    ...
    private fun uploadBuf(i: Int, d: FloatArray, s: Int) {
        ...
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, d.size*4, b, GLES30.GL_DYNAMIC_DRAW)
        GLES30.glEnableVertexAttribArray(i); GLES30.glVertexAttribPointer(i, s, GLES30.GL_FLOAT, false, 0, 0)
    }
    fun updateVerts(d: FloatArray) { vertices=d; if(!init) return; GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER,vbo[0]); val b=ByteBuffer.allocateDirect(d.size*4).order(ByteOrder.nativeOrder()).asFloatBuffer().put(d); b.position(0); GLES30.glBufferSubData(GLES30.GL_ARRAY_BUFFER,0,d.size*4,b) }
